...
(function(){
    ...
    var shouldCustomManageCookie = false;
//    if (customManageCookieRegx && customManageCookieRegx.test(pageUrl)) {
//        shouldCustomManageCookie = true;
//    }
    if (!shouldCustomManageCookie && cookieOriginSetter && cookieOriginGetter) {
        Object.defineProperty(document, 'cookie', {
            ...
            get: function(){
                var cookieOrigin = cookieOriginGetter.apply(document,[]);
                if (typeof window.NebulaEnforceCtokenValue  === 'string' && window.NebulaEnforceCtokenValue.length) {
                	var ctokenExist = false;
	                var ctokenValue = cookieOrigin.replace(/(?:(?:^|.*;\s*)ctoken\s*\=\s*([^;]*).*$)|^.*$/, "$1");
	                if (typeof ctokenValue  === 'string' && ctokenValue.length) {
	                	ctokenExist = true;
	                }

	                if (!ctokenExist) {
	                	cookieOrigin = cookieOrigin + "; ctoken=" + window.NebulaEnforceCtokenValue;
	                }
                }
      ...
      if (shouldCustomManageCookie || window.SHOULDNEBULADORWC) {
          var cookieNameMap = {
                  Version: 'version',
                  Name: 'name',
                  Value: 'value',
        ...
        function getCookieStr() {
              ...
              index = 0, item, result = [], ctokenExist = false;
              for (; index < length; index ++) {
                  item = cookieStore[index];
                  if (!isCookieExpired(item)) {
                	  if (item[cookieNameMap.Name] == 'ctoken' && typeof item[cookieNameMap.Value]  === 'string' && item[cookieNameMap.Value].length > 0) {
                		  ctokenExist = true;
                	  }
                      result.push(item[cookieNameMap.Name] + '=' + item[cookieNameMap.Value])
                  }
              };
              if (!ctokenExist && typeof window.NebulaEnforceCtokenValue  === 'string' && window.NebulaEnforceCtokenValue.length) {
              	   result.push('ctoken='+window.NebulaEnforceCtokenValue);
              }
              return result.join('; ')
          };
    ...
    var sendBeconImplemented = Navigator.prototype.sendBeacon != undefined;
    if (sendBeconImplemented) {
      Navigator.prototype.sendBeacon = (function(fn){
    ...
    FakeFormData.prototype.append = function(name, value, filename){
        ...
        this.appendData.push({"name":name,"value":value,"fileName":filename||filenameInValue, "type":(typeof value)});
        var args = [].slice.call(arguments);
        window.originFormData.prototype.append.apply(this.realFormData, args);
    }

    FakeFormData.prototype.delete = window.originFormData.prototype.delete? function(name){
    	for(var i=0,flag=true,len=this.appendData.length;i<len;flag ? i++ : i){
			if( this.appendData[i]&&this.appendData[i].name==name){
				this.appendData.splice(i,1);
				flag = false;
			} else {
				flag = true;
			}
    	...
    	var args = [].slice.call(arguments);
        window.originFormData.prototype.delete.apply(this.realFormData, args);
    } : undefined;

   	FakeFormData.prototype.entries = window.originFormData.prototype.entries? function(){
    	var args = [].slice.call(arguments);
        return window.originFormData.prototype.entries.apply(this.realFormData, args);
    } : undefined; 

    FakeFormData.prototype.get = window.originFormData.prototype.get? function(){
    	var args = [].slice.call(arguments);
        return window.originFormData.prototype.get.apply(this.realFormData, args);
    } : undefined; 

    FakeFormData.prototype.getAll = window.originFormData.prototype.getAll? function(){
    	var args = [].slice.call(arguments);
        return window.originFormData.prototype.getAll.apply(this.realFormData, args);
    } : undefined; 

    FakeFormData.prototype.has = window.originFormData.prototype.has? function(){
    	var args = [].slice.call(arguments);
        return window.originFormData.prototype.has.apply(this.realFormData, args);
    } : undefined; 

    FakeFormData.prototype.keys = window.originFormData.prototype.keys? function(){
    	var args = [].slice.call(arguments);
        return window.originFormData.prototype.keys.apply(this.realFormData, args);
    } : undefined; 

    FakeFormData.prototype.set = window.originFormData.prototype.set? function(name, value, filename){
    	for(var i=0,flag=true,len=this.appendData.length;i<len;flag ? i++ : i){
			if( this.appendData[i]&&this.appendData[i].name==name){
				this.appendData.splice(i,1);
				flag = false;
    	...
    	var args = [].slice.call(arguments);
        window.originFormData.prototype.set.apply(this.realFormData, args);
    } : undefined; 

    FakeFormData.prototype.values = window.originFormData.prototype.values? function(){
    	var args = [].slice.call(arguments);
        return window.originFormData.prototype.values.apply(this.realFormData, args);
    } : undefined; 
